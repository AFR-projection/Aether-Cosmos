...
import { getActivityScopeId } from "@/shared/lib/activity/activity-store";
...
/** The brand is a proper noun in every locale, so it is not a translation key. */
const BRAND_NAME = "Aether Cosmos";
const BRAND_INITIALS = "ByAFR";
...
function Wordmark() {
  return (
    <p className="truncate text-[13px] font-bold leading-tight tracking-tight">
      {BRAND_NAME} <span className="text-gradient">{BRAND_INITIALS}</span>
    </p>
  );
}
  ...
  return (
    <div className="flex h-full flex-col" suppressHydrationWarning>
          ...
          return (
            ...
            >
              ...
              >
                ...
                {showLabels && <span className="relative z-10">{t(labelKey)}</span>}
                {active && showLabels && (
                  <motion.div
        ...
        {user.role !== "master" && (
          ...
          >
            ...
            >
              ...
              {showLabels && <span className="relative z-10">{t("nav.settings")}</span>}
            </Link>
          </motion.div>
        )}
        ...
        {user.role === "master" && (
          ...
          >
            ...
            >
              ...
              {showLabels && <span className="relative z-10">{t("nav.admin")}</span>}
            </Link>
          </motion.div>
        )}
      </nav>
      ...
      <div className="flex shrink-0 flex-col gap-2 border-t border-border/50 p-3">
        {/* The rail is 72px collapsed, which cannot hold three language chips —
            the selector rides along with the labels. */}
        {showLabels && <LanguageSelector variant="compact" />}
        <div className={cn(
          "flex items-center",
  ...
  return (
    <>
      ...
      <aside
        ...
        suppressHydrationWarning
        aria-label={t("nav.sidebarLandmark")}
      >

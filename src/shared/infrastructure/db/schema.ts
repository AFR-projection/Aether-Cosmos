 ...
 * and we query it via the FTS helpers in @/shared/lib/search/fts.ts.
 */
...
export const users = pgTable(
  "users",
  {
    ...
    /**
     * Digit count of the code above, so the login numpad can draw exactly as many
     * slots as the account actually uses. The hash is argon2, so the length is not
     * recoverable from it, and without this column the pad had to draw the whole
     * 6–10 range — every user with a 6-digit code saw four slots too many.
     *
     * Null means "not known yet" (a code enrolled before this column existed); the
     * login route fills it in on the next successful verification.
     */
    stepCodeLength: integer("step_code_length"),
 ...
 * @/shared/infrastructure/email/crypto.ts). Multiple senders enable priority-ordered failover.
 */
export const mailSenders = pgTable(
  "mail_senders",
  {
    ...
    /** AES-256-GCM ciphertext of the Gmail App Password (see @/shared/infrastructure/email/crypto.ts). */
     ...
     *
     * A literal, not `APP_NAME` from `src/shared/lib/app-version.ts`: drizzle-kit loads this
 ...
 * (AES-256-GCM via @/shared/infrastructure/email/crypto.ts) in `api_key_encrypted` and is NEVER returned

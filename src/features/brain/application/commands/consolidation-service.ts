...
import { memories, type Memory } from "@/shared/infrastructure/db/schema";
...
export async function consolidateBrain(params: {
  ...
  for (const group of duplicates) {
    ...
    for (const loser of losers) {
      ...
      // Archiving alone used to leave the temporal model claiming the duplicate
      // was still active. Keep the explicit link and the scalar provenance in
      // sync so retrieval, timelines, exports, and health checks agree on which
      // memory replaced it. The brain/id predicates prevent a foreign-memory id
      // from ever becoming a cross-tenant supersession pointer.
      ...
      if (transitioned.length !== 1) {
        throw new Error("Consolidation lost its scoped duplicate memory");
      }
      memoriesArchived += 1;

...
describe("detectConflicts", () => {
  ...
  it("requires real overlap, not just a negation somewhere in the brain", () => {
    const conflicts = detectConflicts([
      memory("a", "Database choice", "Postgres on Aiven holds every memory row"),
...
describe("consolidation provenance invariants", () => {
  it("marks every archived duplicate as superseded by the survivor in the same scoped write", () => {
    const source = readFileSync("src/features/brain/application/commands/consolidation-service.ts", "utf8");
    expect(source).toContain('validityState: "superseded"');
    expect(source).toContain("supersededById: survivor.id");

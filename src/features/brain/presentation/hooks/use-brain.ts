...
import { publishBrainChange, subscribeBrainChange } from "@brain/application/commands/brain-sync";
...
/**
 * A failed brain request, carrying the API's machine-readable `code` alongside its
 * prose. Without the code the UI can only show whatever sentence the server sent,
 * which is English; with it, `apiErrorMessage` can prefer a translated
 * `errors.code.*` leaf and fall back to the prose only for codes it has no leaf for.
 */
...
/**
 * Narrows an unknown mutation/query error to the shape `apiErrorMessage` reads.
 * Anything that is not a BrainRequestError (a thrown string, a network TypeError)
 * still yields its message, so the fallback key is only used when there is nothing.
 */
export function requestFailure(error: unknown): { error?: string; code?: string } {
  if (error instanceof BrainRequestError) return { error: error.message, code: error.code };
...
async function get<T>(url: string): Promise<T> {
  ...
  if (!res.success || !res.data) throw new BrainRequestError(res.error ?? "Request failed", res.code);
  return res.data;
}
...
async function send<T>(url: string, method: string, body?: unknown): Promise<T> {
  ...
  if (!res.success) throw new BrainRequestError(res.error ?? "Request failed", res.code);
  ...
  return useMutation({
    mutationFn: async ({ file, apply }: { file: File; apply: boolean }) => {
      ...
      if (!res.success || !res.data)
        throw new BrainRequestError(res.error ?? "Import failed", res.code);
      return res.data;
    },
    onSuccess: (data) => {

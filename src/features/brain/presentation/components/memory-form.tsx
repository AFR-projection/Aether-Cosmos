...
import type { BrainProject, MemoryDraft } from "@brain/presentation/hooks/use-brain";
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <Input
          ...
          placeholder={t("brain.form.titlePlaceholder")}
          autoFocus
        />
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          rows={7}
          placeholder={t("brain.form.contentPlaceholder")}
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div>
          ...
          <select
            ...
            className="h-10 w-full rounded-xl border border-border/60 bg-surface px-3 text-sm text-foreground focus-visible:border-accent/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/15"
          >
            <option value="">{t("brain.form.noProject")}</option>
      ...
      <div>
        <label htmlFor="memory-summary" className="mb-1.5 block text-xs font-medium text-foreground">
          {t("brain.form.summary")}{" "}
          <span className="font-normal text-muted-foreground">{t("brain.form.optional")}</span>
        </label>
        <Input
          id="memory-summary"
          value={summary}
          ...
          placeholder={t("brain.form.summaryPlaceholder")}
        />
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <SliderField
          ...
          label={t("brain.form.importance")}
          hint={t("brain.form.importanceHint")}
          value={importance}
          onChange={setImportance}
        />
        <SliderField
          id="memory-confidence"
          label={t("brain.form.confidence")}
          hint={t("brain.form.confidenceHint")}
          value={confidence}
      ...
      <div>
        <label htmlFor="memory-tags" className="mb-1.5 block text-xs font-medium text-foreground">
          {t("brain.form.tags")}{" "}
          <span className="font-normal text-muted-foreground">{t("brain.form.tagsHint")}</span>
        </label>
        <Input
          id="memory-tags"
          value={tagsText}
          onChange={(event) => setTagsText(event.target.value)}
          placeholder={t("brain.form.tagsPlaceholder")}
        />
      </div>
      ...
      {showChangeReason && (
        <div>
          ...
          >
            {t("brain.form.reason")}{" "}
            <span className="font-normal text-muted-foreground">{t("brain.form.reasonHint")}</span>
          </label>
          <Input
            id="memory-reason"
            value={changeReason}
            ...
            placeholder={t("brain.form.reasonPlaceholder")}
          />
        </div>
      )}

...
import type { GraphModel } from "@brain/presentation/canvas/types";
...
export function GraphContextMenu({
  ...
  const items: Item[] = [];
  if (memoryHref(node)) {
    items.push({ action: "open-note", icon: ExternalLink, label: t("brain.graph.openMemory") });
  }
  items.push({
    action: "open-local",
    icon: GitFork,
    label: isFocal ? t("brain.graph.refitLocal") : t("brain.graph.localFromHere"),
  });
  items.push({ action: "focus", icon: Crosshair, label: t("brain.graph.centreOnNode") });
  items.push({ action: "copy-link", icon: Link2, label: t("brain.graph.copyLink") });
  items.push({ action: "hide", icon: EyeOff, label: t("brain.graph.hideNode"), danger: true });
  ...
  return createPortal(
    <div
      ...
      role="menu"
      tabIndex={-1}
      aria-label={t("brain.graph.actionsFor", { label: node.label })}

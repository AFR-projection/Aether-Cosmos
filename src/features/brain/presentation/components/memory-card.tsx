...
import { memoryTypeLabel } from "@brain/domain/ui-constants";
import type { Memory } from "@brain/presentation/hooks/use-brain";
...
export function MemoryTypeBadge({ type }: { type: string }) {
  ...
  return (
    ...
    >
      {/* Display only — the raw enum is what travels to the API. */}
...
export function ScoreMeter({
  ...
  const percent = formatNumber(pct);
  return (
    <span
      className="flex items-center gap-1.5"
      title={t("brain.card.scoreTitle", { label, percent })}
    >
      <span className="text-[10px] uppercase tracking-wide text-muted-foreground">{label}</span>
      <span
        className="h-1 w-10 overflow-hidden rounded-full bg-muted/50"
        role="img"
        aria-label={t("brain.card.scoreAria", { label, percent })}
      >
        <span
    ...
    >
      ...
      <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2">
        ...
        <span className="flex items-center gap-1 text-[11px] text-muted-foreground">
          ...
          {memory.createdByAgent ? t("brain.by.agent") : t("brain.by.you")}
        </span>
        <span className="flex items-center gap-1 text-[11px] text-muted-foreground">

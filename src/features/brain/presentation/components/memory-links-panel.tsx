...
import { linkVerbLabel, entityTypeLabel, memoryTypeLabel } from "@brain/domain/ui-constants";
import {
  useEntities,
  useLinkMemory,
  useMemories,
...
function LinkRow({
  ...
  const verb = linkVerbLabel(link.linkType, t);
    ...
    <li className="flex items-center gap-2 rounded-xl border border-border/40 px-3 py-2 transition-colors hover:border-accent/30">
          ...
          {link.nodeType &&
            (link.targetType === "memory" ? (
              ...
              <span className="rounded-md border border-border/50 px-1.5 py-0.5 text-[10px] uppercase tracking-wide text-muted-foreground">
                {entityTypeLabel(link.nodeType, t)}
              </span>
            ))}
        </span>
      </span>
      ...
      {onRemove && (
        <Button
          ...
          size="icon-sm"
          disabled={removing}
          aria-label={t("brain.links.removeAria", { type: verb, label: link.label })}
          onClick={onRemove}
        >
          {removing ? (
...
export function MemoryLinksPanel({
  ...
  async function handleRemove(link: MemoryLinkNode) {
    const confirmed = await askConfirm({
      title: t("brain.links.removeTitle", { type: linkVerbLabel(link.linkType, t) }),
      message: t("brain.links.removeBody"),
      confirmText: t("brain.links.removeConfirm"),
    });
    if (!confirmed) return;

    setRemovingId(link.id);
    unlink.mutate(link.id, {
      onSuccess: () => notify({ title: t("brain.links.removed"), tone: "success" }),
      onError: (error) =>
        notify({
          title: error instanceof Error ? error.message : t("brain.links.removeFailed"),
          tone: "error",
        }),
    ...
    <BrainPanel
      ...
      action={
        ...
        >
          ...
          {adding ? t("common.cancel") : t("brain.links.add")}
        </Button>
      }
    >
      {dialogs}
      ...
      {links.isLoading && (
        <div className="space-y-2" aria-busy="true" aria-label={t("brain.links.loading")}>
          {[0, 1].map((row) => (
      ...
      {links.data && (
        <div className="space-y-4">
          <LinkGroup
            heading={t("brain.links.relatedTo")}
            hint={t("brain.links.relatedToHint")}
            empty={t("brain.links.relatedToEmpty")}
            links={relatedTo}
            onRemove={handleRemove}
            removingId={removingId}
          />
          <LinkGroup
            heading={t("brain.links.referencedBy")}
            hint={t("brain.links.referencedByHint")}
            empty={t("brain.links.referencedByEmpty")}
            links={referencedBy}
          />
        </div>
      )}
...
function AddLinkForm({
    ...
    mode === "memory"
      ? (memoryResults.data?.memories ?? [])
          ...
          .map((memory) => ({
            id: memory.id,
            label: memory.title,
            sublabel: memoryTypeLabel(memory.type, t),
          }))
      : (entityResults.data?.entities ?? [])
          ...
          .map((entity) => ({
            id: entity.id,
            label: entity.name,
            sublabel: entityTypeLabel(entity.type, t),
          }));
  ...
  function handleSubmit(event: React.FormEvent) {
    ...
    link.mutate(
      ...
      {
        onSuccess: () => {
          notify({ title: t("brain.links.linked", { label: selected.label }), tone: "success" });
          onDone();
        },
        onError: (error) =>
          notify({
            title: error instanceof Error ? error.message : t("brain.links.createFailed"),
            tone: "error",
          }),
      }
    );
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1" role="group" aria-label={t("brain.links.targetGroup")}>
        {(["memory", "entity"] as const).map((option) => (
          ...
          >
            {option === "memory" ? t("brain.links.targetMemory") : t("brain.links.targetEntity")}
          </button>
        ))}
      </div>
      ...
      <div>
        <label htmlFor="link-search" className="mb-1.5 block text-xs font-medium text-foreground">
          {mode === "memory" ? t("brain.links.findMemory") : t("brain.links.findEntity")}
        </label>
        <Input
          id="link-search"
          value={search}
          ...
          placeholder={
            mode === "memory"
              ? t("brain.links.searchMemories")
              : t("brain.links.searchEntities")
          }
          autoComplete="off"
        />
      </div>
      ...
      {selected ? (
        <p className="flex items-center gap-2 rounded-lg border border-accent/30 bg-accent/5 px-2.5 py-1.5">
          ...
          <Button
            ...
            variant="ghost"
            size="icon-sm"
            aria-label={t("brain.links.clearSelection")}
        ...
        <p className="px-1 text-xs text-muted-foreground">{t("brain.links.searching")}</p>
      ) : options.length ? (
        ...
        >
          {LINK_TYPE_SUGGESTIONS.map((type) => (
            <option key={type} value={type}>
              {linkVerbLabel(type, t)}
            </option>
          ))}
          <option value={CUSTOM}>{t("brain.links.custom")}</option>
        </select>
      </div>
      ...
      {linkType === CUSTOM && (
        <div>
          <label htmlFor="custom-type" className="mb-1.5 block text-xs font-medium text-foreground">
            {t("brain.links.customType")}
          </label>
          <Input
            id="custom-type"
            value={customType}
            onChange={(event) => setCustomType(event.target.value)}
            placeholder={t("brain.links.customPlaceholder")}

...
import type { Brain } from "@/shared/infrastructure/db/schema";
import type { SessionUser } from "@/shared/lib/auth/session";
import { BrainConflictError, BrainForbiddenError, BrainNotFoundError } from "@brain/domain/errors";
 ...
 * @brain/domain/constants.ts is deliberately NOT mocked, so `brain.full` and the
 * write⇒link implication are checked against the one real implementation.
 */

const requireAuthOrApiKey = vi.fn();

vi.mock("@/shared/lib/auth/api-key", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@/shared/lib/auth/api-key")>();
  return {
    ...actual,
...
vi.mock("@brain/application/commands/brain-service", () => ({
...
describe("agent scope algebra", () => {
  ...
  it("honours brain.write ⇒ brain.link, so old grants keep working", async () => {
    // The implication lives in @brain/domain/constants.ts; access.ts must not re-derive it.

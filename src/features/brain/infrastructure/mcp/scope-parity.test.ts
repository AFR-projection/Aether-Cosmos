...
import {
  ...
  requireGrant,
  type McpPrincipal,
} from "@brain/infrastructure/mcp/principal";

/**
 * ONE authorization semantics for both transports.
 *
 * A brain agent can reach its memories two ways: over HTTP through
 * `src/features/brain/infrastructure/access.ts`, or over MCP through `src/features/brain/infrastructure/mcp/principal.ts`. Those
...
/**
 * The REST verdict, as `src/features/brain/infrastructure/access.ts` computes it: `requireAuthOrApiKey`
...
describe("neither path may reintroduce a local scope check", () => {
  const sources = {
    "src/features/brain/infrastructure/access.ts": readFileSync("src/features/brain/infrastructure/access.ts", "utf8"),
    "src/features/brain/infrastructure/mcp/principal.ts": readFileSync("src/features/brain/infrastructure/mcp/principal.ts", "utf8"),
  };

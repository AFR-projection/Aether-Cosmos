...
import { brainAccess, brainAgents, brains } from "@/shared/infrastructure/db/schema";
import { authenticateApiKey, keyHasScope } from "@/shared/lib/auth/api-key";
...
import { BrainForbiddenError, BrainNotFoundError } from "@brain/domain/errors";
 ...
 *
 * Both halves go through the shared helpers in @brain/domain/constants.ts. Rolling
...
export function requireGrant(
  ...
  // Same predicate the REST choke point uses (@brain/infrastructure/access.ts), so a scope

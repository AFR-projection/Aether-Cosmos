...
import { decryptToBlob, isEncryptionMeta, type EncryptionMetaV1 } from "@/shared/lib/crypto/client-encryption";
...
/**
 * Failures are reported as stable codes, never as prose: the download store is
 * read by the activity centre and the downloads widget, which resolve the code
 * through `errorCodeMessage` in whatever language the reader picked. A code the
 * registry does not know falls through as-is, as does a server message and a
 * browser exception — neither can be translated here.
 */
...
export function requestDownload(file: DownloadableFile) {
  if (file.encrypted) {
    if (!isEncryptionMeta(file.encryptionMeta)) {
      ...
      failDownload(id, "DOWNLOAD_ENC_META_MISSING");
      return;
    }
...
export async function saveDecryptedFile(
  ...
  if (!response.ok) throw new Error("DOWNLOAD_FETCH_ENCRYPTED_FAILED");
  ...
  try {
    ...
    const msg = error instanceof Error ? error.message : "DOWNLOAD_SAVE_FAILED";
    failDownload(id, msg);
    throw error;
  }
}
...
export function downloadViewerSource(src: string, fileId: string, fileName: string) {
  if (src.startsWith("blob:")) {
    ...
    try {
      ...
      failDownload(id, error instanceof Error ? error.message : "DOWNLOAD_SAVE_FAILED");
    }
    return;
  }
...
export function downloadFile(fileId: string, fileName: string) {
  ...
  try {
    ...
    const msg = error instanceof Error ? error.message : "DOWNLOAD_START_FAILED";
    failDownload(id, msg);
  }
}
...
export async function downloadZip(ids: string[], label = "download.zip") {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const msg = json?.code ?? json?.error ?? "ZIP_FAILED";
      failDownload(id, msg);
      return;
    }
    ...
    const msg = error instanceof Error ? error.message : "ZIP_DOWNLOAD_FAILED";
    failDownload(id, msg);
  }
}
...
export async function requestFolderArchive(folderId: string, folderName: string) {
  ...
  try {
    ...
    if (!response.ok || !json?.data?.job) {
      const msg = json?.code ?? json?.error ?? "ARCHIVE_FAILED";
    ...
    for (let attempt = 0; attempt < 300; attempt += 1) {
      if (job.status === "ready") {
        ...
        if (!url) throw new Error("ARCHIVE_URL_NOT_READY");
        const anchor = document.createElement("a");
        anchor.href = url;
      ...
      if (job.status === "failed" || job.status === "expired") {
        throw new Error(job.errorMessage ?? "ARCHIVE_BUILD_FAILED");
      }
    ...
    throw new Error("ARCHIVE_TIMEOUT");
  } catch (error) {
    const msg = error instanceof Error ? error.message : "ARCHIVE_DOWNLOAD_FAILED";
    failDownload(downloadId, msg);
  }
}
...
async function getArchiveStatus(id: string): Promise<{ job: ArchiveJobClient; downloadUrl?: string }> {
  ...
  if (!response.ok || !json?.data?.job) throw new Error(json?.code ?? json?.error ?? "ARCHIVE_STATUS_FAILED");

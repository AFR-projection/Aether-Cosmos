 ...
 *
 * The panels in `@files/presentation/components/editors` decide nothing: they collect a draft and hand it
...
import type { PreviewKind } from "./detect-preview-type";
/**
 * Type-only, and pointed at the modules rather than at `@/shared/lib/i18n` — the barrel is
 * `"use client"`, and this file is also imported by the edit route and the worker.
 * A `import type` is erased, so nothing client-only can follow it into a server bundle.
 */
import type { TranslationKey } from "@/shared/lib/i18n/dictionary";
import type { TParams } from "@/shared/lib/i18n/format";

export type Size = { width: number; height: number };

/**
 * Why something cannot be done, as a dictionary key rather than a sentence.
 *
 * This module runs in the browser, in the route handler and in the worker, and only the
 * first of those knows which language the reader chose. So a refusal names the wording and
 * carries whatever it interpolates; the panel resolves it at render, which also means a
 * refusal produced before a language switch reads in the language on screen afterwards.
 */
export type EditRefusal = { key: TranslationKey; params?: TParams };
...
export type CropAspect = {
  id: string;
  /**
   * The ratio as printed — `4:3` reads the same in every language. `null` for the free
   * crop, the only chip that is a word: the panel translates that one itself.
   */
  label: string | null;
  ratio: number | null;
};
 ...
 */
export function outputSizeError(size: ResizePlan): EditRefusal | null {
  if (
    ...
    return { key: "files.edit.size.whole" };
  }
  if (size.width > EDIT_MAX_DIMENSION || size.height > EDIT_MAX_DIMENSION) {
    return { key: "files.edit.size.tooWide", params: { max: EDIT_MAX_DIMENSION } };
  }
  if (size.width * size.height > EDIT_MAX_OUTPUT_PIXELS) {
    return {
      key: "files.edit.size.tooManyPixels",
      params: { max: Math.round(EDIT_MAX_OUTPUT_PIXELS / 1_000_000) },
    };
  }
  return null;
}
...
export function buildImageEditRequest(input: {
  ...
  if (!(natural.width >= 1) || !(natural.height >= 1)) {
    return { ok: false, reason: { key: "files.edit.stillLoading" } };
  }
  if (!hasImageChanges(draft)) {
    return { ok: false, reason: { key: "files.edit.nothingToSave" } };
  }
...
export const IMAGE_CONVERT_FORMATS: readonly {
  ...
  { id: "jpeg", label: "JPEG", noteKey: "files.edit.format.jpeg" },
  { id: "png", label: "PNG", noteKey: "files.edit.format.png" },
  { id: "webp", label: "WebP", noteKey: "files.edit.format.webp" },
  { id: "avif", label: "AVIF", noteKey: "files.edit.format.avif" },
];
 ...
 */
export function trimError(window: TrimWindow, duration: number | null): EditRefusal | null {
  const { startSeconds, endSeconds } = window;
  if (!Number.isFinite(startSeconds) || !Number.isFinite(endSeconds) || startSeconds < 0) {
    return { key: "files.edit.trim.noWindow" };
  }
  if (endSeconds - startSeconds < MIN_TRIM_SECONDS) {
    return { key: "files.edit.trim.tooShort", params: { seconds: MIN_TRIM_SECONDS } };
  }
  if (endSeconds > TRIM_MAX_SECONDS) {
    return { key: "files.edit.trim.tooLong" };
  }
  if (duration !== null && Number.isFinite(duration) && duration > 0) {
    if (startSeconds >= duration) return { key: "files.edit.trim.startPastEnd" };
    if (endSeconds - startSeconds >= duration - 0.05) {
      return { key: "files.edit.trim.wholeClip" };
    }
  }
  return null;
}

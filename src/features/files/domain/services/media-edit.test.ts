...
describe("outputSizeError", () => {
  ...
  it("rejects a fractional or empty size", () => {
    expect(outputSizeError({ width: 100.5, height: 100 })?.key).toBe("files.edit.size.whole");
    expect(outputSizeError({ width: 0, height: 100 })?.key).toBe("files.edit.size.whole");
  });

  it("rejects a side past the dimension ceiling, and says which ceiling", () => {
    expect(outputSizeError({ width: EDIT_MAX_DIMENSION + 1, height: 10 })).toEqual({
      key: "files.edit.size.tooWide",
      params: { max: EDIT_MAX_DIMENSION },
    });
  });
  ...
  it("rejects an area past the pixel ceiling even when both sides pass", () => {
    ...
    expect(outputSizeError({ width: side, height: side })?.key).toBe(
      "files.edit.size.tooManyPixels"
    );
  });
});
...
describe("buildImageEditRequest", () => {
  ...
  it("refuses a draft that asks for nothing", () => {
    ...
    expect(plan).toEqual({ ok: false, reason: { key: "files.edit.nothingToSave" } });
  });
  ...
  it("asks for a copy only when told to", () => {
    ...
    if (!inPlace.ok) throw new Error(inPlace.reason.key);
    ...
    if (!copy.ok) throw new Error(copy.reason.key);
  ...
  it("names a conversion above a rotation, and a flip last of all", () => {
    ...
    if (!converted.ok) throw new Error(converted.reason.key);
    ...
    if (!rotated.ok) throw new Error(rotated.reason.key);
...
describe("IMAGE_CONVERT_FORMATS", () => {
  it("offers each format once, with something to read next to it", () => {
    ...
    for (const option of IMAGE_CONVERT_FORMATS) {
      ...
      // The note is a dictionary key, not a sentence — `check:i18n` proves it resolves.
      expect(option.noteKey).toBe(`files.edit.format.${option.id}`);
    }
  });
});
...
describe("trimError", () => {
  ...
  it("rejects a clip shorter than the floor, and says what the floor is", () => {
    expect(trimError({ startSeconds: 5, endSeconds: 5.2 }, 60)).toEqual({
      key: "files.edit.trim.tooShort",
      params: { seconds: MIN_TRIM_SECONDS },
    });
  });

  it("rejects an inverted window", () => {
    expect(trimError({ startSeconds: 20, endSeconds: 5 }, 60)?.key).toBe(
      "files.edit.trim.tooShort"
    );
  });

  it("rejects marks that are not numbers", () => {
    expect(trimError({ startSeconds: NaN, endSeconds: 5 }, 60)?.key).toBe(
      "files.edit.trim.noWindow"
    );
    expect(trimError({ startSeconds: -1, endSeconds: 5 }, 60)?.key).toBe(
      "files.edit.trim.noWindow"
    );
  });

  it("rejects a window past the hard ceiling", () => {
    expect(trimError({ startSeconds: 0, endSeconds: TRIM_MAX_SECONDS + 1 }, null)?.key).toBe(
      "files.edit.trim.tooLong"
    );
  });

  it("rejects a start beyond the end of the clip", () => {
    expect(trimError({ startSeconds: 90, endSeconds: 120 }, 60)?.key).toBe(
      "files.edit.trim.startPastEnd"
    );
  });
  ...
  it("refuses a trim that would rewrite the file for no change", () => {
    ...
    expect(trimError({ startSeconds: 0, endSeconds: 60 }, 60)?.key).toBe(
      "files.edit.trim.wholeClip"
    );
  });

 ...
 *
 * Everything that can be decided without a DOM lives in `src/features/files/domain/services/folder-tree.ts`;
...
import {
  ...
  type TreeRoot,
  type TreeRow,
} from "@files/domain/services/folder-tree";
import {
  TREE_WIDTH_MAX,
  TREE_WIDTH_MIN,
  clampTreeWidth,
...
function ResizeHandle({
  ...
  return (
    <div
      ...
      aria-label={t("files.tree.resize")}
      aria-valuenow={width}
...
export function FolderTreeSidebar({
  ...
  return (
    <aside
      aria-label={t("files.tree.paneLabel")}
      style={{ width }}
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden rounded-xl border border-border bg-surface">
        ...
        >
          ...
          {rows.length === 0 && !loading && (
            <p className="px-2 py-2.5 text-xs text-muted-foreground">{t("files.tree.empty")}</p>
          )}
          {truncated && (
            <p className="px-2 py-2.5 text-xs text-muted-foreground">
              {t("files.tree.truncated")}
            </p>
          )}
        </div>
      </div>

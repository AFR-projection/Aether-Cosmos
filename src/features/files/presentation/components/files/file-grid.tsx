...
import { cn, getMimeCategory } from "@/shared/lib/utils";
import { FileTypeIcon, getAccentColor, getGradientFallback } from "@/shared/lib/file-type-utils";
import { fileTypeKey, useFormat, useT, type TranslationKey, type Translator } from "@/shared/lib/i18n";
import { sortFiles } from "@files/domain/services/sort";
import type { DragData } from "@files/domain/services/drag-move";
...
import { isLiteMode } from "@/shared/lib/system/lite-mode";
 ...
 * `src/shared/lib/auth/permissions.ts` — the API refuses regardless, this just stops the UI from
...
/**
 * `t` is passed in rather than read from a hook: this builds a plain array, and
 * the three call sites below are already inside components that hold a
 * translator.
 */
function buildFileMenuItems(
  ...
  if (trash) {
    ...
    return [
      { id: "restore", label: t("recycleBin.restore"), icon: RotateCcw, onClick: () => onAction("restore", file) },
      { id: "delete", label: t("files.list.deletePermanently"), icon: Trash2, danger: true, shortcut: "Del", onClick: () => onAction("delete", file) },
    ];
  }
  const items: FloatingMenuItem[] = [
    { id: "download", label: t("common.download"), icon: Download, onClick: () => onAction("download", file) },
  ];
  ...
  if (caps.canOwnerOnlyFlags) {
    items.push({ id: "share", label: t("files.list.share"), icon: Share2, onClick: () => onAction("share", file) });
  }
  items.push({ id: "clip-copy", label: t("common.copy"), icon: Copy, shortcut: "Ctrl C", onClick: () => onAction("clip-copy", file) });
  if (caps.canEdit) {
    items.push(
      { id: "clip-cut", label: t("files.list.cut"), icon: Scissors, shortcut: "Ctrl X", onClick: () => onAction("clip-cut", file) },
      { id: "rename", label: t("common.rename"), icon: Pencil, shortcut: "F2", onClick: () => onAction("rename", file) },
      { id: "move", label: t("files.list.moveTo"), icon: FolderInput, shortcut: "M", onClick: () => onAction("move", file) },
    );
  }
  if (caps.canOwnerOnlyFlags) {
    items.push({ id: "favorite", label: file.isFavorite ? t("files.list.unfavorite") : t("files.list.favorite"), icon: Star, onClick: () => onAction("favorite", file) });
  }
  if (caps.canEdit) {
    items.push(
      { id: "duplicate", label: t("files.list.duplicate"), icon: CopyPlus, onClick: () => onAction("duplicate", file) },
      { id: "delete", label: t("files.list.trash"), icon: Trash2, danger: true, shortcut: "Del", onClick: () => onAction("delete", file) },
    );
  }
  return items;
}
...
function SortHeader({ labelKey, sortKey, current, order, onSort }: SortHeaderProps) {
  const t = useT();
  ...
  return (
    <button
      ...
      // The state goes in the label instead, which screen readers do announce. The
      // column word is interpolated rather than lowercased: `toLowerCase` is not a
      // safe transform for translated text, and Chinese has no case to change.
      aria-label={
        active
          ? t(order === "asc" ? "files.list.sortedAscending" : "files.list.sortedDescending", {
              column: label,
            })
          : t("files.list.sortBy", { column: label })
      }
      className={cn(
      ...
      <div className="rounded-xl border border-border bg-surface p-3 shadow-xl">
        ...
        <div className="space-y-1 text-xs">
          <div className="flex justify-between items-center">
            <span className="text-muted-foreground">{t("files.list.colSize")}</span>
            <span className="font-mono font-medium text-foreground">{formatBytes(file.sizeBytes)}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-muted-foreground">{t("files.list.colModified")}</span>
            <span className="text-foreground">{formatDate(file.updatedAt, "short")}</span>
          </div>
          {(file.isFavorite || file.encrypted) && (
            <div className="flex items-center gap-2 pt-0.5">
              {file.isFavorite && <span className="flex items-center gap-1 text-warning-ink"><Star className="h-3 w-3 fill-current" />{t("files.list.favorite")}</span>}
              {file.encrypted && <span className="flex items-center gap-1 text-accent-ink"><Lock className="h-3 w-3" />{t("files.list.encrypted")}</span>}
            </div>
          )}
        </div>
      </div>
          ...
          <div ref={loadMoreRef} className="flex justify-center py-8">
            {loadingMore ? (
              <div className="flex items-center gap-2 text-sm text-muted-foreground" role="status">
                <Spinner size="xs" />
                {t("files.list.loadingMore")}
              </div>
            ) : (
              <Button variant="ghost" size="sm" onClick={loadMore}>
                {t("files.list.loadMore")}
              </Button>
            )}
          </div>
        )}
    ...
    <div className="rounded-2xl border border-border/40 bg-surface">
      ...
      >
        <button
          ...
          aria-label={allSelected ? t("files.list.deselectAll") : t("files.list.selectAll")}
        >
          <div className={cn(
        ...
        <SortHeader labelKey="common.name" sortKey="name" current={sortBy} order={sortOrder} onSort={onSort} />
        <SortHeader labelKey="files.list.colSize" sortKey="size" current={sortBy} order={sortOrder} onSort={onSort} />
        <span className="hidden sm:block">
          <SortHeader labelKey="files.list.colModified" sortKey="date" current={sortBy} order={sortOrder} onSort={onSort} />
        </span>
        <span className="hidden md:block">
          <SortHeader labelKey="files.list.colType" sortKey="type" current={sortBy} order={sortOrder} onSort={onSort} />
        </span>
            ...
            return (
              ...
              >
                {loadingMore ? (
                  ...
                  <Button variant="ghost" size="sm" onClick={loadMore} className="cursor-pointer">{t("files.list.loadMore")}</Button>
                )}
              </div>
            );
          }
    ...
    >
      ...
      {file.isFavorite && (
        <div className="absolute -top-1.5 -right-1.5 z-30 flex h-5 w-5 items-center justify-center rounded-full bg-warning shadow-md ring-2 ring-background">
          ...
          <span className="sr-only">{t("files.list.favorite")}</span>
        </div>
      )}
      {file.encrypted && (
        <div
        ...
        >
          ...
          <span className="sr-only">{t("files.list.encryptedAes")}</span>
        </div>
      )}
      ...
      <button
        ...
        aria-label={t("files.list.selectFile", { name: file.name })}
      >
      ...
      <div className="px-2.5 pt-2 pb-2.5">
        ...
        <button
          ...
          aria-label={t("files.list.openFile", { name: file.name })}
        ...
        <div className="mt-1 flex items-center justify-between">
          ...
          {file.isNote
            ? <span className="rounded bg-accent/12 px-1.5 py-0.5 text-xs font-semibold uppercase tracking-wide text-accent-ink">{t("common.note")}</span>
      ...
      <FloatingActionMenu
        ...
        items={buildFileMenuItems(file, trash, onFileAction, caps, t)}
        placement="context"
        menuLabel={t("files.list.actionsFor", { name: file.name })}
        header={{ title: file.name, subtitle: `${t(fileTypeKey(file.mimeType))} · ${formatBytes(file.sizeBytes)}` }}
      />
    </div>
  );
});
...
const ListRow = memo(function ListRow({
  ...
  const menuItems = buildFileMenuItems(file, trash, onFileAction, caps, t);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2.5 min-w-0 py-2 pr-2 sm:pr-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <span className="text-xs text-muted-foreground sm:hidden">
            {formatBytes(file.sizeBytes)} · {t(fileTypeKey(file.mimeType))}
          </span>
        </div>
        <div className="hidden sm:flex items-center gap-1.5 shrink-0">
          ...
          {file.encrypted && (
            <>
              ...
              <span className="sr-only">{t("files.list.encrypted")}</span>
            </>
          )}
          {file.isNote && <span className="rounded bg-accent/10 px-1.5 py-0.5 text-xs font-semibold uppercase tracking-wide text-accent-ink">{t("common.note")}</span>}
        </div>
      </div>
      ...
      {/* Type */}
      <span className="text-xs text-muted-foreground truncate hidden lg:block">{t(fileTypeKey(file.mimeType))}</span>
      ...
      <div className="flex justify-end" onClick={(e) => e.stopPropagation()}>
        <Button
          ...
          aria-label={t("files.list.moreActionsFor", { name: file.name })}
...
function CardActions({ file, trash, onAction, caps }: {
  ...
  const t = useT();
  const menu = useFloatingMenu();
  const menuItems = buildFileMenuItems(file, trash, onAction, caps, t);
    ...
    <>
      <div className="flex items-center gap-1">
        ...
        <Button
          ...
          onClick={() => menu.toggle(file.id)}
          title={t("files.list.moreActions")}
          aria-label={t("files.list.moreActions")}
          aria-expanded={menu.isOpen(file.id)}
        >

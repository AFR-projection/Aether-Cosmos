...
export function PptxViewer({ src, fileName, fileId }: PptxViewerProps) {
  ...
  /* A flag, not a sentence — the words are chosen at render, in the language the
     reader has by then. */
  ...
  const message = error ?? (renderFailed ? t("files.viewer.pptx.failed") : null);
      ...
      <div className="relative min-h-0 flex-1 overflow-auto">
        ...
        {(loading || rendering) && (
          <div className="absolute inset-0 bg-viewer-stage">
            <ViewerLoading
              label={t(
                loading ? "files.preview.loading.presentation" : "files.viewer.pptx.rendering"
              )}
            />
          </div>
        )}

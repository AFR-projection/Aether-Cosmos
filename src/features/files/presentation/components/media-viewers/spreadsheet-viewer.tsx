...
export function SpreadsheetViewer({ src, fileName, fileId }: SpreadsheetViewerProps) {
  ...
  /* A flag, not a sentence: `XLSX.read` fails once and the words are chosen below. */
  ...
  if (loading) return <ViewerLoading label={t("files.preview.loading.spreadsheet")} />;

  const message = error ?? (parseFailed ? t("files.viewer.sheet.failed") : null);
  if (message) {
    return (
      <ViewerMessage
        icon={Table2}
  ...
  if (!workbook || grid.rows.length === 0) {
    return (
      <ViewerMessage
        ...
        hint={
          workbook
            ? t("files.viewer.sheet.emptySheet", { name: sheetName })
            : t("files.viewer.sheet.noSheets")
        }
        onDownload={handleDownload}
      />
    );
  ...
  return (
    <div className="flex h-full flex-col bg-surface">
      ...
      {workbook.SheetNames.length > 1 && (
        <div
          role="tablist"
          aria-label={t("files.viewer.sheet.tabs")}
      ...
      <div className="min-h-0 flex-1 overflow-auto">
        <table className="w-full min-w-max border-collapse text-xs">
          <caption className="sr-only">
            {t("files.viewer.sheet.caption", {
              name: fileName,
              sheet: sheetName,
              rows: t("files.viewer.rows", { count: grid.rows.length }),
              columns: t("files.viewer.columns", { count: colCount }),
            })}
          </caption>

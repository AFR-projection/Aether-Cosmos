...
export function ViewerDownloadButton({ onDownload }: { onDownload: () => void }) {
  ...
  return (
    <Button
      variant="ghost"
      size="icon"
      aria-label={t("files.preview.downloadFile")}
      onClick={onDownload}
    >
        ...
        <div className="flex flex-wrap items-center justify-center gap-2">
          {onRetry && (
            <Button variant="secondary" size="sm" onClick={onRetry}>
              <RefreshCw className="h-3.5 w-3.5" aria-hidden="true" /> {t("errorPages.tryAgain")}
            </Button>
          )}
          {onDownload && (
            <Button size="sm" onClick={onDownload}>
              <Download className="h-3.5 w-3.5" aria-hidden="true" /> {t("common.download")}
            </Button>
          )}
        </div>
      )}

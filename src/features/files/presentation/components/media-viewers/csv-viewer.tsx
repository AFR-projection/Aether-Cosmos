...
export function CsvViewer({ src, fileName }: CsvViewerProps) {
  ...
  /* Whether it failed, not what to say — the sentence is chosen at render. */
  ...
  if (loading) return <ViewerLoading label={t("files.preview.loading.table")} />;
  ...
  if (failed) {
    return (
      <ViewerMessage
        ...
        hint={t("files.viewer.csv.failed")}
        onRetry={() => {
          setLoading(true);
          setAttempt((n) => n + 1);
        }}
  ...
  if (rows.length === 0) {
    return (
      <ViewerMessage
        icon={Table2}
        title={t("files.viewer.csv.emptyTitle")}
        hint={t("files.viewer.csv.emptyHint")}
      />
    );
  }

  // Counted once and read three times: the badge, the caption and nothing else.
  const rowsLabel = t("files.viewer.rows", { count: totalRows });
  const colsLabel = t("files.viewer.columns", { count: colCount });
    ...
    <div className="flex h-full flex-col bg-surface">
      <ViewerBar
        ...
        meta={
          <span className="flex shrink-0 items-center gap-1.5">
            <Badge tone="neutral">{`${rowsLabel} · ${colsLabel}`}</Badge>
            {truncated && (
              <Badge tone="warning">
                <AlertTriangle className="h-3 w-3" aria-hidden="true" />
                {t("files.viewer.truncatedRows", { count: MAX_ROWS })}
              </Badge>
            )}
          </span>
        }
      >
        <Button
          variant="ghost"
          size="icon"
          aria-label={t(copied ? "common.copied" : "files.viewer.csv.copy")}
          ...
          <thead className="sticky top-0 z-10 bg-surface-elevated/95 backdrop-blur-sm">
            <tr>
              ...
              {Array.from({ length: colCount }).map((_, i) => (
                ...
                >
                  {headers[i] || t("files.viewer.csv.columnN", { n: i + 1 })}
                </th>
              ))}
            </tr>
          </thead>

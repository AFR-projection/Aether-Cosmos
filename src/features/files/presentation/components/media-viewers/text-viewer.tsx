...
import { getLanguage, highlightLine } from "@files/domain/services/text-highlight";
import { isSaveShortcut } from "@files/domain/services/note-draft";
import {
  TEXT_EDIT_MAX_BYTES,
  clampTextForPreview,
  isTextEditable,
...
/** A size in kilobytes. `KB` is a unit, so it reads the same in every language. */
...
export function TextViewer({
  ...
  /* Whether the fetch failed, not what to say about it. */
  ...
  const save = useCallback(async () => {
    ...
    if (draftOverLimit) {
      setSaveError(
        t("files.viewer.text.tooLarge", {
          size: formatKb(draftBytes),
          limit: formatKb(TEXT_EDIT_MAX_BYTES),
        })
      );
      return;
    }
    ...
    try {
      ...
      if (!res.success) {
        setSaveError(apiErrorMessage(res, t, "files.viewer.text.saveFailed"));
        return;
      }
      ...
      setSaveError(t("files.viewer.text.saveFailedNetwork"));
    } finally {
      setSaving(false);
    }
  }, [fileId, saving, dirty, draft, draftBytes, draftOverLimit, fileVersion, onSaved, t]);
  ...
  if (loadFailed) {
    return (
      <ViewerMessage
        ...
        hint={t("files.viewer.text.failed")}
        tone="danger"
        onRetry={() => {
  ...
  if (!loaded) return <ViewerLoading label={t("files.viewer.text.loading")} />;
    ...
    <div className="flex h-full flex-col">
      <div className="flex shrink-0 items-center justify-between gap-2 border-b border-border/30 bg-muted/20 px-4 py-2">
        <div className="flex min-w-0 items-center gap-2 text-xs text-muted-foreground">
          ...
          <span className="shrink-0">{t("files.viewer.text.lines", { count: lineLabel })}</span>
          ...
          {!editing && loaded.truncated && (
            <Badge tone="warning">
              ...
              {t("files.viewer.text.truncated")}
            </Badge>
          )}
          {editing && dirty && <Badge tone="warning">{t("files.viewer.text.unsaved")}</Badge>}
          {editing && !dirty && !saving && savedAt !== null && (
            <Badge tone="success">{t("files.viewer.text.saved")}</Badge>
          )}
        </div>
        ...
        <div className="flex shrink-0 items-center gap-1">
          {!editing && (
            <Button
              ...
              aria-label={t(
                wordWrap ? "files.viewer.text.disableWrap" : "files.viewer.text.enableWrap"
              )}
              title={t(
                wordWrap ? "files.viewer.text.disableWrap" : "files.viewer.text.enableWrap"
              )}
          ...
          <Button
            ...
            size="icon-sm"
            onClick={handleCopy}
            aria-label={t("files.viewer.text.copyContents")}
            title={t("files.viewer.text.copyContents")}
          >
            {copied ? (
          ...
          {!editing && editable && (
            <Button variant="secondary" size="sm" onClick={startEditing}>
              <Pencil className="h-3.5 w-3.5" aria-hidden="true" /> {t("files.viewer.text.edit")}
            </Button>
          )}
          {editing && (
            <>
              ...
              <Button
                ...
                disabled={saving}
                aria-label={t("files.viewer.text.closeEditor")}
                title={t("files.viewer.text.closeEditor")}
              >
      ...
      {showSizeWarning && (
        <div className="flex items-start gap-2 border-b border-warning/10 bg-warning/5 px-4 py-1.5 text-xs text-warning-ink">
          ...
          <span>
            {t("files.viewer.text.sizeNotice", { size: formatKb(loaded.totalBytes) })}
            {loaded.truncated &&
              ` ${t("files.viewer.text.showingFirst", { size: formatKb(TEXT_EDIT_MAX_BYTES) })}`}
            {canEdit &&
              !!fileId &&
              !loaded.editable &&
              ` ${t("files.viewer.text.readOnlyOver", { size: formatKb(TEXT_EDIT_MAX_BYTES) })}`}
          </span>
        </div>
      )}
      ...
      {confirmDiscard && (
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-warning/15 bg-warning/5 px-4 py-2 text-xs text-warning-ink">
          <span>{t("files.viewer.text.unsavedChanges")}</span>
          <div className="flex items-center gap-2">
            ...
            <Button variant="destructive" size="sm" onClick={leaveEditing}>
              {t("files.viewer.text.discardChanges")}
            </Button>
          </div>
        </div>
      )}
      ...
      {editing ? (
        <div className="flex min-h-0 flex-1">
          ...
          <textarea
            ...
            wrap="off"
            aria-label={t("files.viewer.text.editAria", { name: fileName })}

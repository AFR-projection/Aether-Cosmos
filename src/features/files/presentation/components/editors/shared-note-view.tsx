...
/**
 * A save that did not get through, recorded as *which* failure rather than as a
 * sentence — the wording is picked at render, so an attempt that failed before the
 * reader switched language still reads in the language on screen. The share endpoint
 * answers with prose only, so there is no error code to prefer over it.
 */
type RequestFailure = { kind: "api"; error?: string } | { kind: "network" };
...
export function SharedNoteView({ token, content, canEdit }: SharedNoteViewProps) {
  ...
  const saveNow = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      if (!res.ok || body.success === false) {
        setSaveError({ kind: "api", error: body.error });
        return false;
      }
  ...
  const saveLabelKey: TranslationKey =
    saveState === "saving"
      ? "common.saving"
      : saveState === "error"
      ...
      {canEdit && draftOffer && (
        <div className="note-banner note-banner--card mb-4" data-tone="warning" role="status">
          ...
          <span className="min-w-0 flex-1">
            {/* One sentence, timestamp included: keying it as a prefix and a suffix around
                the <strong> that used to wrap the time would fix an order the languages
                do not share. */}
        ...
        {canEdit ? (
          <>
            ...
            <span className="inline-flex items-center gap-1 rounded-full border border-accent/30 bg-accent/10 px-2 py-0.5 font-medium text-accent-ink">
              <Pencil aria-hidden className="h-3 w-3" /> {t("files.note.editable")}
            </span>
            <button
              type="button"
              ...
              title={t("files.note.saveShortcut", { shortcut: isMac ? "⌘S" : "Ctrl+S" })}
            >
              {saveState === "saving" ? (
          ...
          <span className="inline-flex items-center gap-1 rounded-full border border-border/50 bg-muted/20 px-2 py-0.5 font-medium text-muted-foreground">
            <Lock aria-hidden className="h-3 w-3" /> {t("files.note.readOnly")}
          </span>
        )}
      </div>

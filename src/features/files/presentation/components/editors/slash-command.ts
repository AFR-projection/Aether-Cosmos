...
/* Names and descriptions are dictionary keys: this list is built when the module loads,
   which is long before anyone has picked a language, so `slash-menu.tsx` resolves them
   at render instead. The keywords are invisible search aliases, kept in English on
   purpose: one spelling to remember whatever the interface is set to. */
export const SLASH_ITEMS: SlashItem[] = [
  { titleKey: "files.note.block.text", descKey: "files.note.block.textDesc", icon: "Type", keywords: ["paragraph", "text", "body"],
    run: (e, r) => e.chain().focus().deleteRange(r).setParagraph().run() },
  { titleKey: "files.note.block.heading1", descKey: "files.note.block.heading1Desc", icon: "Heading1", keywords: ["h1", "title", "heading"],
    run: (e, r) => e.chain().focus().deleteRange(r).toggleHeading({ level: 1 }).run() },
  { titleKey: "files.note.block.heading2", descKey: "files.note.block.heading2Desc", icon: "Heading2", keywords: ["h2", "subheading", "heading"],
    run: (e, r) => e.chain().focus().deleteRange(r).toggleHeading({ level: 2 }).run() },
  { titleKey: "files.note.block.heading3", descKey: "files.note.block.heading3Desc", icon: "Heading3", keywords: ["h3", "heading"],
    run: (e, r) => e.chain().focus().deleteRange(r).toggleHeading({ level: 3 }).run() },
  { titleKey: "files.note.block.bulletList", descKey: "files.note.block.bulletListDesc", icon: "List", keywords: ["ul", "bullet", "list", "unordered"],
    run: (e, r) => e.chain().focus().deleteRange(r).toggleBulletList().run() },
  { titleKey: "files.note.block.numberedList", descKey: "files.note.block.numberedListDesc", icon: "ListOrdered", keywords: ["ol", "number", "numbered", "ordered"],
    run: (e, r) => e.chain().focus().deleteRange(r).toggleOrderedList().run() },
  { titleKey: "files.note.block.todoList", descKey: "files.note.block.todoListDesc", icon: "ListChecks", keywords: ["todo", "task", "check", "checklist"],
    run: (e, r) => e.chain().focus().deleteRange(r).toggleTaskList().run() },
  { titleKey: "files.note.block.quote", descKey: "files.note.block.quoteDesc", icon: "Quote", keywords: ["blockquote", "quote", "citation"],
    run: (e, r) => e.chain().focus().deleteRange(r).toggleBlockquote().run() },
  { titleKey: "files.note.block.codeBlock", descKey: "files.note.block.codeBlockDesc", icon: "Code2", keywords: ["code", "snippet", "pre"],
    run: (e, r) => e.chain().focus().deleteRange(r).toggleCodeBlock().run() },
  { titleKey: "files.note.block.divider", descKey: "files.note.block.dividerDesc", icon: "Minus", keywords: ["hr", "divider", "rule", "separator"],
    run: (e, r) => e.chain().focus().deleteRange(r).setHorizontalRule().run() },
];

/**
 * A "/qu" has to match "Quote" in the language on screen as well as by alias, so the
 * name is resolved here rather than read off the item. The translator is built per
 * call: this runs inside a ProseMirror plugin, where there is no render to capture one
 * from, and a stale one would keep filtering against the previous language.
 */
        ...
        render: () => {
          ...
          return {
            onStart: (props) => {
              ...
              // 90 is the floating-menu tier in the LAYER scale (@/ui/primitives/modal.tsx),

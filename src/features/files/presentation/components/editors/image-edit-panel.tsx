...
import type { EditRefusal } from "@files/domain/services/media-edit";
...
/**
 * Why the last attempt got nowhere — recorded as *which* failure, never as a sentence.
 *
 * The wording is chosen at render instead, so a refusal that happened before the reader
 * switched language still reads in the language on screen. `api` keeps the server's own
 * reply untouched for `apiErrorMessage`, which prefers a known error code over its prose.
 */
type SaveFailure =
  | { kind: "refusal"; refusal: EditRefusal }
  ...
  return (
    ...
    >
      <div
        ...
        role="group"
        tabIndex={0}
        aria-label={t("files.edit.image.cropRegion", {
...
export default function ImageEditPanel({
  ...
  const save = async (mode: "replace" | "copy") => {
    ...
    if (!built.ok) {
      setError({ kind: "refusal", refusal: built.reason });
      return;
    }
    ...
    try {
      ...
      if (!res.success || !res.data) {
        setError({ kind: "api", error: res.error, code: res.code });
        return;
      }

      if (mode === "replace") reset();
      notify({
        title: t(mode === "copy" ? "files.edit.image.copyTitle" : "files.edit.image.savedTitle"),
        description:
          mode === "copy"
            ? t("files.edit.image.copyBody", {
                name: res.data.name ?? t("files.edit.image.copyFallbackName"),
              })
            : t("files.edit.image.savedBody", {
                name: fileName,
      ...
      <aside className="flex w-full shrink-0 flex-col gap-5 overflow-y-auto border-t border-border/60 bg-card p-4 lg:w-72 lg:border-l lg:border-t-0">
        <section>
          <h3 className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
            {t("files.edit.image.rotateFlip")}
          </h3>
          <div className="mt-2 grid grid-cols-2 gap-2">
            <Button variant="secondary" size="sm" onClick={() => rotateBy(-90)}>
              <RotateCcw className="h-3.5 w-3.5" />
              {t("files.edit.image.left")}
            </Button>
            <Button variant="secondary" size="sm" onClick={() => rotateBy(90)}>
              <RotateCw className="h-3.5 w-3.5" />
              {t("files.edit.image.right")}
            </Button>
            <Button
              variant="secondary"
              size="sm"
        ...
        <section>
          ...
          <div className="mt-2 grid grid-cols-3 gap-1.5">
            {CROP_ASPECTS.map((option) => (
              ...
              >
                {option.label ?? t("files.edit.image.aspectFree")}
              </button>
            ))}
          </div>
          <p className="mt-2 flex items-start gap-1.5 text-[11px] leading-relaxed text-muted-foreground">
            <Crop className="mt-0.5 h-3 w-3 shrink-0" />
            {t("files.edit.image.cropHint")}
          </p>
        </section>
        ...
        <section>
          <div className="flex items-baseline justify-between">
            <h3 className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
              {t("files.edit.image.resize")}
            </h3>
            <span className="font-mono text-[11px] text-muted-foreground">
              {resizeTarget === null ? t("files.edit.image.original") : `${resizeTarget} px`}
            </span>
          </div>
          <div className="mt-2 grid grid-cols-3 gap-1.5">
            {RESIZE_PRESETS.map((preset) => {
              ...
              return (
                <button
                  ...
                  title={
                    upscale
                      ? t("files.edit.image.presetTooLarge")
                      : t("files.edit.image.preset", { px: preset })
                  }
                >
                  {preset}
                </button>
            ...
            >
              {t("files.edit.image.original")}
            </button>
          </div>
          <div className="mt-2">
            <EditSlider
              label={t("files.edit.image.longestEdge")}
              ...
              valueText={
                output
                  ? t("files.edit.image.outputPixels", {
                      width: output.width,
                      height: output.height,
                    })
                  : t("files.edit.image.outputWaiting")
              }
              onChange={(value) =>
        ...
        <section>
          <div className="flex items-baseline justify-between">
            <h3 className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
              {t("files.edit.image.format")}
            </h3>
            <span className="font-mono text-[11px] text-muted-foreground">
              {sourceFormat ? sourceFormat.toUpperCase() : t("files.edit.image.formatOther")}
            </span>
          </div>
          <div className="mt-2 grid grid-cols-3 gap-1.5">
            {IMAGE_CONVERT_FORMATS.map((option) => {
              ...
              return (
                <button
                  ...
                  title={own ? t("files.edit.image.alreadyFormat") : t(option.noteKey)}
                >
                  {option.label}
                </button>
              );
            ...
            >
              {t("files.edit.image.keepFormat")}
            </button>
          </div>
          <p className="mt-2 text-[11px] leading-relaxed text-muted-foreground">
            {target
              ? t("files.edit.image.convertNote", {
                  note: t(target.noteKey),
                  extension: encoderForFormat(target.id).extension,
                })
              : t("files.edit.image.keptAsIs")}
          </p>
        </section>
        ...
        <section>
          <div className="flex items-center justify-between">
            <h3 className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
              {t("files.edit.image.compress")}
            </h3>
            <button
              type="button"
              role="switch"
              aria-checked={quality !== null}
              aria-label={t("files.edit.image.recompress")}
          ...
          {quality !== null && (
            <div className="mt-2">
              <EditSlider
                label={t("files.edit.image.quality")}
                min={40}
                max={100}
                value={quality}
                valueText={t("files.edit.image.qualityValue", { value: quality })}
                onChange={setQuality}
              />
              <p className="mt-1 text-[11px] text-muted-foreground">
                {t(
                  targetIsPng
                    ? "files.edit.image.qualityPng"
                    : "files.edit.image.qualityLossy",
                  { value: quality }
                )}
              </p>
            </div>
        ...
        <section className="mt-auto space-y-2 border-t border-border/60 pt-4">
          <dl className="space-y-1 text-[11px]">
            <div className="flex justify-between gap-2">
              <dt className="text-muted-foreground">{t("files.edit.image.source")}</dt>
              <dd className="font-mono text-muted-foreground">
                {natural ? `${natural.width} × ${natural.height}` : "—"}
              </dd>
            </div>
            <div className="flex justify-between gap-2">
              <dt className="text-muted-foreground">{t("files.edit.image.output")}</dt>
          ...
          {error ? (
            <p role="alert" className="text-[11px] leading-relaxed text-danger-ink">
              {error.kind === "refusal"
                ...
                : error.kind === "api"
                  ? apiErrorMessage(error, t, "files.edit.image.saveFailed")
                  : t("files.edit.image.sendFailed")}
            </p>
          ) : dirty && plan && !plan.ok ? (
            <p role="status" className="text-[11px] leading-relaxed text-warning-ink">
              {t(plan.reason.key, plan.reason.params)}
            </p>
          ) : null}

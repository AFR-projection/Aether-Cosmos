 ...
 * sentence — the wording is picked at render, so an attempt that failed before the
...
/** The trim can also be refused locally, before anything is sent. */
type TrimFailure = { kind: "refusal"; refusal: EditRefusal } | RequestFailure;
  ...
  return (
    <div>
      ...
      <input
        ...
        disabled={disabled}
        aria-valuetext={t("files.edit.clip.handleAt", {
          label,
...
export default function MediaTrimPanel({
  ...
  const [error, setError] = useState<TrimFailure | null>(null);
  ...
  const [audioError, setAudioError] = useState<RequestFailure | null>(null);

  const dirty =
    !queued &&
    duration !== null &&
    (clip.startSeconds > 0.05 || clip.endSeconds < duration - 0.05);
  const invalid: EditRefusal | null =
    duration === null ? { key: "files.edit.clip.waiting" } : trimError(clip, duration);
  ...
  const submit = async () => {
    ...
    try {
      ...
      setQueued(true);
      notify({
        title: t("files.edit.clip.queuedTitle"),
        description: t("files.edit.clip.queuedBody", {
          name: fileName,
  ...
  const extractAudio = async () => {
    ...
    try {
      ...
      setAudioQueued(true);
      notify({
        title: t("files.edit.clip.audio.queuedTitle"),
        description: t("files.edit.clip.audio.queuedBody", { name: fileName }),
        tone: "success",
      });
      onQueued?.();
    } catch {
      setAudioError({ kind: "network" });
    } finally {
      setExtracting(false);
    }
  };
    ...
    <div className="flex h-full min-h-0 flex-col lg:flex-row">
      ...
      <aside className="flex w-full shrink-0 flex-col gap-5 overflow-y-auto border-t border-border/60 bg-card p-4 lg:w-72 lg:border-l lg:border-t-0">
          ...
          {container === null ? (
            ...
            <>
              ...
              <div className="mt-3 flex gap-2">
                ...
                >
                  {t("files.edit.clip.playSelection")}
                </Button>
                <Button
                  variant="ghost"
                  size="sm"
        ...
        {isVideo ? (
          <section>
            <h3 className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
              {t("files.edit.clip.audio.heading")}
            </h3>
            <Button
              variant="secondary"
              size="sm"
            ...
            >
              ...
              {t(
                audioQueued
                  ? "files.edit.clip.audio.queued"
                  : "files.edit.clip.audio.extract"
              )}
            </Button>
            {audioError ? (
              <p role="alert" className="mt-2 text-[11px] leading-relaxed text-danger-ink">
                {audioError.kind === "api"
                  ? apiErrorMessage(audioError, t, "files.edit.clip.audio.failed")
                  : t("files.edit.clip.audio.sendFailed")}
              </p>
            ) : audioQueued ? (
              <p role="status" className="mt-2 text-[11px] leading-relaxed text-success-ink">
                {t("files.edit.clip.audio.running")}
              </p>
            ) : (
              <p className="mt-2 text-[11px] leading-relaxed text-muted-foreground">
                {t("files.edit.clip.audio.hint")}
              </p>
            )}
          </section>
        ) : null}
        ...
        <section className="mt-auto space-y-2 border-t border-border/60 pt-4">
          <dl className="space-y-1 text-[11px]">
            ...
            <div className="flex justify-between gap-2">
              <dt className="text-muted-foreground">{t("files.edit.clip.keeping")}</dt>
          ...
          {error ? (
            <p role="alert" className="text-[11px] leading-relaxed text-danger-ink">
              {error.kind === "refusal"
                ...
                : error.kind === "api"
                  ? apiErrorMessage(error, t, "files.edit.clip.queueFailed")
                  : t("files.edit.clip.sendFailed")}
            </p>
          ) : queued ? (
            <p role="status" className="text-[11px] leading-relaxed text-success-ink">
              {t("files.edit.clip.running")}
            </p>
          ) : dirty && invalid ? (
          ...
          >
            ...
            {t(queued ? "files.edit.clip.queuedTitle" : "files.edit.clip.submit")}
          </Button>
          <p className="text-[11px] leading-relaxed text-muted-foreground">
            {t("files.edit.clip.footnote", { seconds: MIN_TRIM_SECONDS })}
          </p>
        </section>
      </aside>
    </div>

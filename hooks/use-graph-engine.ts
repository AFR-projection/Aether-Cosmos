...
import { ForceSimulation } from "@/lib/brain/graph/simulation";
import type { ForceSettings, GraphModel } from "@/lib/brain/graph/types";
import type { GraphView } from "@/lib/brain/graph/view";
import type { ForceRequest, ForceResponse } from "@/lib/brain/graph/worker-protocol";
...
export function useGraphEngine(input: {
  ...
  useEffect(() => {
    ...
    try {
      created = new Worker(new URL("../lib/brain/graph/force.worker.ts", import.meta.url), {
        type: "module",
      });
    } catch {
      created = null;

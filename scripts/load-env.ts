...
/**
 * Environment loading for the operational scripts, with NO production dependency.
 *
 * These scripts (migrations, backfills, verifiers, master bootstrap) are run on the
 * server, where `npm ci --omit=dev` is normal — so `import "dotenv/config"` crashes with
 * MODULE_NOT_FOUND before a single line runs. Depending on a devDependency to reach the
 * database is a deployment trap, not a convenience.
 *
 ...
 *
 * Import it FIRST in a script, exactly where `dotenv/config` used to sit: anything that
 * reads `process.env` at module scope must see the values already applied.
 */

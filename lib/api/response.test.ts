...
import * as boundedBody from "@/lib/api/body";
 ...
 *  - `BodyTooLargeError` existed TWICE, declared in both `lib/api/body.ts` and
 *    `lib/api/read-body.ts`. `instanceof` compares identity, so the 413 branch
...
describe("handleApiError — body errors", () => {
  ...
  describe("there is exactly one class per name", () => {
    ...
    it("an oversized body read through lib/api/body still reaches the 413 branch", async () => {
    ...
    it("an oversized body read through lib/api/read-body reaches it too", async () => {
      const request = new Request("https://example.test/api/anything", {
        method: "POST",
        body: "y".repeat(500),
      });
    ...
    it("a malformed body read through lib/api/body reaches the 400 branch", async () => {
      const request = new Request("https://example.test/api/anything", {
        method: "POST",

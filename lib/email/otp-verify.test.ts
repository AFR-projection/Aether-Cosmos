...
vi.mock("@/lib/email/mailer", () => ({ deliverMail: async () => true }));
vi.mock("@/lib/email/log", () => ({ recordEmailLog: () => undefined }));

const { verifyOTP, OTP_MAX_ATTEMPTS } = await import("@/lib/email/email-service");
const { hashOTP } = await import("@/lib/email/otp-utils");

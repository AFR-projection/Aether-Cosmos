...
import { estimateTokens, usableTokenBudget, TOKEN_MODEL } from "./tokens";
import {
...
describe("the SQL and the constants keep the engine's invariants", () => {
  const source = readFileSync("lib/brain/context-engine.ts", "utf8");
...
describe("brain_recall is left alone", () => {
  const recall = readFileSync("lib/brain/recall.ts", "utf8");

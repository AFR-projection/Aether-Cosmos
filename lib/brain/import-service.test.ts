...
describe("ownership comes from the authenticated target, never from the archive", () => {
  const fullArchive = () =>
    archive({
      ...
      entities: [
        ...
        { id: "e2", name: "Neon", type: "organization" },
  ...
  it("re-mints every memory id instead of reusing the archive's", async () => {
    await run(archive({ memories: [mem({ id: "m1" }), mem({ id: "m2", title: "Neon" })] }));
...
describe("entities merge on (brain_id, name, type)", () => {
  ...
  it("treats the same name under two types as two nodes", async () => {
    ...
    const result = await run(
      archive({
        memories: [mem()],
        entities: [
          { id: "e1", name: "Neon", type: "organization" },
          { id: "e2", name: "Neon", type: "technology" },
        ] as ParsedArchive["entities"],
        relationships: [
  ...
  it("does not reuse a row that shares the name but not the type", async () => {
    ...
    // called "Neon" into the company.
    resolveEntities([{ name: "Neon", type: "organization" }]);

    const result = await run(
      archive({
        memories: [mem()],
        entities: [{ id: "e1", name: "Neon", type: "person" }] as ParsedArchive["entities"],
      })
    );
...
describe("BUG-2 — archive metadata survives the write", () => {
  ...
  it("keeps memory link metadata and adds provenance beside it", async () => {
    await run(
      archive({
        memories: [mem({ id: "m1" }), mem({ id: "m2", title: "Neon" })],
        memoryLinks: [
          {
            sourceMemoryId: "m1",
            targetType: "memory",
...
function fullArchive(): ParsedArchive {
  ...
  resolveEntities([{ name: "Postgres", type: "technology" }, { name: "Neon", type: "organization" }]);
  resolveTags(["deploy", "database"]);
  return archive({
    memories: [
      mem({ id: "m1", projectId: "p1", tags: ["deploy", "database"], importance: 0.82 }),
      mem({ id: "m2", title: "Neon branching", tags: ["deploy"] }),
    ],
    memoryVersions: [
...
describe("export → import round trip", () => {
  ...
  function sourceData() {
    return {
      memories: [
        ...
        sourceMemory({
          id: SRC_M2,
          title: "Neon branching for previews",
          content: "Each preview deploy gets its own Neon branch.",
  ...
  it("keeps every link pointing at the same endpoints it did in the source brain", async () => {
    ...
    expect(links[0]).toMatchObject({
      sourceMemoryId: ids.get("Neon branching for previews"),
      targetType: "memory",
  ...
  it("still has every entity, with curated metadata intact beside provenance", async () => {
    ...
    expect(entities.map((row) => row.name)).toEqual(["Postgres", "Neon"]);
